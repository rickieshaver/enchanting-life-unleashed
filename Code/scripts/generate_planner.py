...
def get_phase(d, full_date, new_date):
    df = (d - full_date).days
    dn = (d - new_date).days
    if df == 0:              return "full_moon",  "Full Moon"
    if 1 <= df <= 6:         return "wan_gib",    "Waning Gibbous"
    if df == 7:              return "last_qtr",   "Last Quarter"
    if 8 <= df <= 13:        return "wan_cres",   "Waning Crescent"
    if dn == 0:              return "new_moon",   "New Moon"
    if 1 <= dn <= 6:         return "wax_cres",   "Waxing Crescent"
    if dn == 7:              return "first_qtr",  "First Quarter"
    if 8 <= dn <= 13:        return "wax_gib",    "Waxing Gibbous"
    if -7 <= df <= -1:       return "wax_gib",    "Waxing Gibbous"
    if df == -8:             return "first_qtr",  "First Quarter"
    if -14 <= df <= -9:      return "wax_cres",   "Waxing Crescent"
    if dn >= 14:             return "wax_gib",    "Waxing Gibbous"
    return None, "—"
...
def build_weeks(month_num, full_date, new_date):
    ...
    for i, (s, e) in enumerate(bounds, 1):
        ...
        for dn2 in range(s, e+1):
            ...
            phase_key, pname = get_phase(d, full_date, new_date)
            days.append((WD[d.weekday()], dn2, phase_key, pname))
...
PHASES = [
  { "emoji":"🌑","key":"new_moon","name":"NEW MOON","subtitle":"Intentions · Beginnings · Planting Seeds",
  ...
  { "emoji":"🌒","key":"wax_cres","name":"WAXING CRESCENT","subtitle":"Momentum · Action · First Steps",
  ...
  { "emoji":"🌓","key":"first_qtr","name":"FIRST QUARTER","subtitle":"Decisions · Commitment · Push Through",
  ...
  { "emoji":"🌔","key":"wax_gib","name":"WAXING GIBBOUS","subtitle":"Refinement · Trust · Almost There",
  ...
  { "emoji":"🌕","key":"full_moon","name":"FULL MOON","subtitle":"Illumination · Celebration · Release",
  ...
  { "emoji":"🌖","key":"wan_gib","name":"WANING GIBBOUS","subtitle":"Gratitude · Integration · Sharing",
  ...
  { "emoji":"🌗","key":"last_qtr","name":"LAST QUARTER","subtitle":"Forgiveness · Surrender · Closure",
  ...
  { "emoji":"🌘","key":"wan_cres","name":"WANING CRESCENT","subtitle":"Rest · Reflection · Renewal",
...
.wl-s { height:1px; background:rgba(109,46,70,0.13); margin:28px 0; }
...
.wl-ds { height:0.7px; background:rgba(109,46,70,0.25); margin:32px 0; }
...
def phase_icon_svg(phase_key, size=48, lit_color="#EDB74D", dark_color="#6D2E46", ring_color="none"):
    """Custom branded moon phase icon — mathematical SVG, no emoji."""
    import math
    r = (size - 2) / 2
    cx = size / 2
    cy = size / 2
    ...
    angle, phase_type = phase_config.get(phase_key, (0, None))

    base = f'<circle cx="{cx}" cy="{cy}" r="{r:.2f}" fill="{{fill}}" stroke="{ring_color}" stroke-width="1.5"/>'
    ring = f'<circle cx="{cx}" cy="{cy}" r="{r:.2f}" fill="none" stroke="{ring_color}" stroke-width="1.5"/>'

    if phase_type is None:
        fill = dark_color if angle == 0 else lit_color
        return f'<svg viewBox="0 0 {size} {size}" width="{size}" height="{size}" xmlns="http://www.w3.org/2000/svg">{base.format(fill=fill)}</svg>'

    if phase_type == "wax":
        tx = abs(math.cos(math.radians(angle))) * r
        ts = 1 if angle > 90 else 0
        path = f"M {cx},{top_y:.2f} A {r:.2f},{r:.2f} 0 0,1 {cx},{bot_y:.2f} A {tx:.2f},{r:.2f} 0 0,{ts} {cx},{top_y:.2f}"
    else:
        af = angle - 180
        tx = abs(math.cos(math.radians(af))) * r
        ts = 0 if af < 90 else 1
        path = f"M {cx},{top_y:.2f} A {r:.2f},{r:.2f} 0 0,0 {cx},{bot_y:.2f} A {tx:.2f},{r:.2f} 0 0,{ts} {cx},{top_y:.2f}"

    return f'<svg viewBox="0 0 {size} {size}" width="{size}" height="{size}" xmlns="http://www.w3.org/2000/svg"><circle cx="{cx}" cy="{cy}" r="{r:.2f}" fill="{dark_color}" stroke="{ring_color}" stroke-width="1.5"/><path d="{path}" fill="{lit_color}"/>{ring}</svg>'


def pi(key, size=16):
    """Inline phase icon — SVG with vertical-align:middle for use inside text/flex rows."""
    if not key: return ""
    return phase_icon_svg(key, size=size).replace(
        '<svg ', f'<svg style="display:inline-block;vertical-align:middle;" ')
...
def chk_rows(n, dark=False, spacing=None):
    box = "chk-box-d" if dark else "chk-box"
    line = "chk-line-d" if dark else "chk-line"
    style = f' style="margin-bottom:{spacing}"' if spacing else ""
    return "".join(f'<div class="chk-item"{style}><div class="{box}"></div><div class="{line}"></div></div>' for _ in range(n))
...
def page_how_it_works():
    sections = [
        (f'{pi("new_moon",18)} New Moon · Days 1–3',"Set your intention. Plant your seed. Write your vision. This is the most potent time for new beginnings, fresh starts, and bold declarations."),
        (f'{pi("wax_cres",18)} Waxing Crescent · Days 3–7',"Take action. Build momentum. The sliver of light grows — your intentions need movement to manifest. Act before you feel ready."),
        (f'{pi("first_qtr",18)} First Quarter · Days 7–10',"Face the challenge. Make the decision. Resistance is normal. Push through. Friction is fuel, not failure."),
        (f'{pi("wax_gib",18)} Waxing Gibbous · Days 10–13',"Refine and trust. You're almost at peak power. Make small adjustments. Don't undo your progress with perfectionism."),
        (f'{pi("full_moon",18)} Full Moon · Day 14–15',"Celebrate and release. Peak illumination. Honor your progress. Let go of what blocks your next level."),
        (f'{pi("wan_gib",18)} Waning Gibbous · Days 15–19',"Integrate and share. Harvest the lessons. Practice gratitude. Share your wisdom generously."),
        (f'{pi("last_qtr",18)} Last Quarter · Days 19–22',"Forgive and complete. Tie up loose ends. Forgive yourself and others. Completion creates space for creation."),
        (f'{pi("wan_cres",18)} Waning Crescent · Days 22–29',"Rest and renew. The void before rebirth. Your magic regenerates in the dark. Rest is sacred, not lazy."),
    ]
    boxes = ""
...
def page_lunar_calendar():
    ...
    for m in MONTHS:
        ...
        rows += f'''<tr>
          ...
          <td>{pi("full_moon",14)} {fd}<br><span class="cal-sign">{m["full_sign"]} · {m["moon_name"]}</span></td>
          <td>{pi("new_moon",14)} {nd}<br><span class="cal-sign">{m["new_sign"]}</span></td>
        </tr>'''
    return f"""
<div class="page page-dark">
  {brackets()}
...
<div class="page page-dark" style="display:flex;align-items:center;justify-content:center;">
  ...
  <div style="text-align:center;padding:40px;">
    ...
    <div style="margin-top:40px;display:flex;justify-content:center;gap:14px;opacity:0.55;">{pi("new_moon",32)}{pi("wax_cres",32)}{pi("first_qtr",32)}{pi("wax_gib",32)}{pi("full_moon",32)}{pi("wan_gib",32)}{pi("last_qtr",32)}{pi("wan_cres",32)}</div>
  </div>
</div>"""
...
<div class="page page-dark">
  {brackets()}
  <div style="position:absolute;top:30px;right:40px;opacity:.4;">{phase_icon_svg(p["key"],size=80)}</div>
  <div class="cp">
  ...
  <div class="cp-sm">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:14px;">
      <div class="mo-moon-box">
        <div class="mo-moon-label">{pi("full_moon",14)} Full Moon</div>
      ...
      <div class="mo-moon-box">
        <div class="mo-moon-label">{pi("new_moon",14)} New Moon</div>
...
def page_weekly_planner(m, week_data, tasks, quote, page_num):
    ...
    days_html = ""
    for wd, dnum, phase_key, pname in week_data["days"]:
        days_html += f"""
        <div class="wp-day-card">
          <div class="wp-day-top">
            <span class="wp-dname">{wd}</span>
            <span class="wp-dnum">{dnum}</span>
            <span class="wp-phase" style="display:flex;align-items:center;gap:4px;">{pi(phase_key,12)} {pname}</span>
          </div>
    ...
    tasks_html = ""
    for t in tasks:
        tasks_html += f'<div class="task-item"><span class="task-emoji">{pi(t["key"],14)}</span><div><div class="task-text">{t["name"]}</div><div class="task-pg">\u2192 page {t["page"]}</div></div></div>'
...
<div class="page page-cream">
  ...
  <div class="cp-sm">
    ...
    {fields_html}
    <div class="lbl-b" style="margin-top:-10px;border-bottom:none;padding-bottom:0;">Boundaries I'm Setting</div>
    <div style="padding-top:28px;">{chk_rows(4, spacing="22px")}</div>
...
<div class="page page-dark">
  ...
  <div class="cp-sm">
    ...
    <div class="dark-date">{pi("full_moon",16)} {full_str} · Moon in {m["full_sign"]}</div>
    ...
    <div style="background:#D5B9B2;border:.5px solid rgba(109,46,70,0.15);border-radius:4px;padding:20px 24px;margin-top:14px;">
      <div style="font-family:'Poppins',sans-serif;font-size:13px;font-weight:600;letter-spacing:.15em;color:#6D2E46;text-transform:uppercase;border-bottom:.5px solid rgba(109,46,70,0.25);padding-bottom:8px;margin-bottom:6px;">Reflection &amp; Release · After the Ritual</div>
      {wlines(6,"wl-ds")}
    </div>
  </div>
...
<div class="page page-cream">
  ...
  <div class="cp-sm">
    ...
    <div class="cream-date">{pi("new_moon",16)} {new_str}</div>
...
def build_html():
    ...
    for i, m in enumerate(MONTHS):
        ...
        w1_tasks = [
            {"key":"first_qtr","name":"First Quarter Worksheet","page": base+2},
            {"key":"wax_gib",  "name":"Waxing Gibbous Worksheet","page": base+3},
            {"key":"full_moon","name":f"{m['moon_name']} Full Moon Ritual","page": base+4},
        ]
        w2_tasks = [
            {"key":"wan_gib","name":"Waning Gibbous Worksheet","page": base+6},
        ]
        w3_tasks = [
            {"key":"last_qtr","name":"Last Quarter Worksheet","page": base+8},
            {"key":"wan_cres","name":"Waning Crescent Worksheet","page": base+9},
            {"key":"new_moon","name":"New Moon Intention","page": base+11},
        ]
        w4_tasks = [
            {"key":"new_moon","name":"New Moon Intention","page": base+11},
        ]
